...
describe("<Footer />", () => {
  ...
  it("Should verify that the components at the very least render the text “Copyright”", () => {
    const wrapper = mount(<Footer />);
    expect(wrapper.find("div.footer p")).toHaveLength(1);
    expect(wrapper.find("div.footer p").text()).toContain("Copyright");
  });

  it("Should verify that the link is not displayed when user is logged out within context", () => {
    const wrapper = mount(
